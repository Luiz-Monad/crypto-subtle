import { __module as ssh } from './_virtual/ssh.js';
...
(function (module) {
	...
	var SSH_EC_P256 = '00000013 65 63 64 73 61 2d 73 68 61 32 2d 6e 69 73 74 70 32 35 36'.replace(/\s+/g, '').toLowerCase();
	// 19  e  c  d  s  a  -  s  h  a  2  -  n  i  s  t  p  3  8  4
	var SSH_EC_P384 = '00000013 65 63 64 73 61 2d 73 68 61 32 2d 6e 69 73 74 70 33 38 34'.replace(/\s+/g, '').toLowerCase();
	...
	SSH.packSsh = function (jwk) {
	  ...
	  var ssh = Enc.hexToUint8(('00 00 00 13 65 63 64 73 61 2d 73 68 61 32 2d 6e 69 73 74 70' + a + '00 00 00 08 6e 69 73 74 70' + a + '00 00 00' + b + '04' + x + y).replace(/\s+/g, '').toLowerCase());
	  return typ + ' ' + Enc.bufToBase64(ssh) + ' ' + comment;
	}; 
} (ssh));

var sshExports = ssh.exports;
