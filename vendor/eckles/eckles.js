import { __module as eckles } from './_virtual/eckles.js';
import { encodingExports } from './encoding.js';
import { pemExports } from './pem.js';
import { sshExports } from './ssh.js';
...
(function (module) {
	...
	EC.generate = function (opts) {
	  return Promise.resolve().then(function () {
	    ...
	    return new Promise(function (resolve, reject) {
	      return require$$4.generateKeyPair(typ, {
	...
	EC.importSync = function importEcSync(opts) {
	  ...
	  if (opts["public"]) {
	    if (true !== opts["public"]) {
	      throw new Error("options.public must be either `true` or `false` not (" + _typeof(opts["public"]) + ") '" + opts["public"] + "'");
	    }
	    delete jwk.d;
	  }
	  return jwk;
	...
	EC.exportSync = function (opts) {
	  if (!opts || !opts.jwk || 'object' !== _typeof(opts.jwk)) {
	  ...
	  if (!jwk.d) {
	    if (!format || -1 !== ['spki', 'pkix'].indexOf(format)) {
	      ...
	      throw new Error("options.format must be 'spki' or 'ssh' for public EC keys, not (" + _typeof(format) + ") " + format);
	    }
	  } else {
	...
	EC.__thumbprint = function (jwk) {
	  ...
	  var buf = require$$4.createHash('sha256')
