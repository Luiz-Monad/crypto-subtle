import { __module as rsa } from './_virtual/rsa.js';
import require$$0 from 'keypairs';
import { rsaCsr } from './rsa-csr/index.js';
import { __require as requireGeneratePrivkey } from './generate-privkey.js';
...
(function (module) {
	...
	RSA.generateKeypair = function (options, cb, extra1, extra2) {
	  ...
	  try {
	    var keypair = requireGeneratePrivkey()(length, exponent);
	...
	RSA.signJws = RSA.generateJws = RSA.generateSignatureJws = RSA.generateSignatureJwk = function (keypair, header, protect, payload) {
	  ...
	  var signer = require$$4.createSign("RSA-SHA256");
	  signer.update(raw);
	  return {
	    header: header,
	...
	RSA._thumbprint = function (keypair) {
	  ...
	  var base64Digest = require$$4.createHash('sha256').update(input).digest('base64');
