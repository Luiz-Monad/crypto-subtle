...
mode.ccm = {
  ...
  encrypt: function (prf, plaintext, iv, adata, tlen) {
    ...
    // compute the tag
    tag = mode.ccm._computeTag(prf, plaintext, iv, adata, tlen, L);

    // encrypt
    out = mode.ccm._ctrMode(prf, out, iv, tag, tlen, L);
  ...
  decrypt: function (prf, ciphertext, iv, adata, tlen) {
    ...
    // check the tag
    tag2 = mode.ccm._computeTag(prf, out.data, iv, adata, tlen, L);
  ...
  _computeTag: function (prf, plaintext, iv, adata, tlen, L) {
    ...
    mac = mode.ccm._macAdditionalData(prf, adata, iv, tlen, w.bitLength(plaintext) / 8, L);
  ...
  _ctrMode: function (prf, data, iv, tag, tlen, L) {
    ...
    for (i = 0; i < l; i += 4) {
      if (i > n) {
        mode.ccm._callProgressListener(i / l);
        n += p;
      }
      ctr[3]++;
      enc = prf.encrypt(ctr);
