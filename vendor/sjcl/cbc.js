  ...
  mode.cbc = {
    ...
    encrypt: function encrypt(prp, plaintext, iv, adata) {
      ...
      if (bitArray.bitLength(iv) !== 128) {
        throw new exception.invalid("cbc iv must be 128 bits");
      }
      var i,
        w = bitArray,
        xor = w._xor4,
        ...
        output = [];
      if (bl & 7) {
        throw new exception.invalid("pkcs#5 padding only works for multiples of a byte");
      }
    ...
    decrypt: function decrypt(prp, ciphertext, iv, adata) {
      ...
      if (bitArray.bitLength(ciphertext) & 127 || !ciphertext.length) {
        throw new exception.corrupt("cbc ciphertext must be a positive multiple of the block size");
      }
      var i,
        w = bitArray,
        xor = w._xor4,
      ...
      bi = output[i - 1] & 255;
      if (bi === 0 || bi > 16) {
        throw new exception.corrupt("pkcs#5 padding corrupt");
      }
      bo = bi * 0x1010101;
