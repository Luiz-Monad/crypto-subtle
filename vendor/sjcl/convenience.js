...
var arrayBuffer = sjcl.arrayBuffer;
var mode = sjcl.mode;
...
sjcl.json = {
  ...
  _encrypt: function (password, plaintext, params, rp) {
    ...
    if (typeof p.salt === "string") {
      p.salt = codec.base64.toBits(p.salt);
    }
    if (typeof p.iv === "string") {
      p.iv = codec.base64.toBits(p.iv);
    }
    ...
    if (typeof password === "string") {
      tmp = misc.cachedPbkdf2(password, p);
    ...
    if (typeof plaintext === "string") {
      plaintext = codec.utf8String.toBits(plaintext);
    }
    if (typeof adata === "string") {
      p.adata = adata = codec.utf8String.toBits(adata);
    }
    prp = new cipher[p.cipher](password);
    ...
    if (p.mode === "ccm" && sjcl.arrayBuffer && sjcl.arrayBuffer.ccm && plaintext instanceof ArrayBuffer) {
      p.ct = arrayBuffer.ccm.encrypt(prp, plaintext, p.iv, adata, p.ts);
    } else {
      p.ct = mode[p.mode].encrypt(prp, plaintext, p.iv, adata, p.ts);
    }
  ...
  _decrypt: function (password, ciphertext, params, rp) {
    ...
    if (typeof adata === "string") {
      adata = codec.utf8String.toBits(adata);
    }
    prp = new cipher[p.cipher](password);

    /* do the decryption */
    if (p.mode === "ccm" && sjcl.arrayBuffer && sjcl.arrayBuffer.ccm && p.ct instanceof ArrayBuffer) {
      ct = arrayBuffer.ccm.decrypt(prp, p.ct, p.iv, p.tag, adata, p.ts);
    } else {
      ct = mode[p.mode].decrypt(prp, p.ct, p.iv, adata, p.ts);
    }
      ...
      if (obj.hasOwnProperty(i)) {
        ...
        switch (typeof obj[i]) {
          ...
          case 'string':
            ...
            break;
          case 'object':
            out += '"' + codec.base64.fromBits(obj[i], 0) + '"';
            break;
          default:
  ...
  decode: function (str) {
    ...
    for (i = 0; i < a.length; i++) {
      ...
      if (m[3] != null) {
        ...
        out[m[2]] = m[2].match(/^(ct|adata|salt|iv)$/) ? codec.base64.toBits(m[4]) : unescape(m[4]);
      } else if (m[5] != null) {
        out[m[2]] = m[5] === 'true';
      }
    }
...
sjcl.misc.cachedPbkdf2 = function (password, obj) {
  ...
  c = cp[iter] = cp[iter] || {
    firstSalt: obj.salt && obj.salt.length ? obj.salt.slice(0) : random.randomWords(2, 0)
  };
  salt = obj.salt === undefined ? c.firstSalt : obj.salt;
  c[salt] = c[salt] || misc.pbkdf2(password, salt, obj.iter);
  return {
    key: c[salt].slice(0),
    salt: salt.slice(0)
  };
