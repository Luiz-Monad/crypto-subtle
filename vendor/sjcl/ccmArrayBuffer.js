...
arrayBuffer.ccm = {
  ...
  compat_encrypt: function compat_encrypt(prf, plaintext, iv, adata, tlen) {
    ...
    ct = codec.arrayBuffer.toBits(encrypted_obj.ciphertext_buffer);
    ct = bitArray.clamp(ct, ol * 8);
  ...
  encrypt: function encrypt(prf, plaintext_buffer, iv, adata, tlen, ol) {
    ...
    //set up defaults
    adata = adata || [];
    tlen = tlen || arrayBuffer.ccm.defaults.tlen;
    ...
    mac = arrayBuffer.ccm._computeTag(prf, plaintext_buffer, iv, adata, tlen, ol, L);

    //encrypt the plaintext and the mac 
    //returns the mac since the plaintext will be left encrypted inside the buffer
    mac = arrayBuffer.ccm._ctrMode(prf, plaintext_buffer, iv, mac, tlen, L);
  ...
  decrypt: function decrypt(prf, ciphertext_buffer, iv, tag, adata, tlen, ol) {
    ...
    //decrypt the buffer
    mac = arrayBuffer.ccm._ctrMode(prf, ciphertext_buffer, iv, tag, tlen, L);
    mac2 = arrayBuffer.ccm._computeTag(prf, ciphertext_buffer, iv, adata, tlen, ol, L);
  ...
  _ctrMode: function _ctrMode(prf, data_buffer, iv, mac, tlen, L) {
    ...
    if (data_buffer.byteLength !== 0) {
      ...
      for (i = 0; i < data.byteLength; i += 16) {
        if (i > n) {
          mode.ccm._callProgressListener(i / data_buffer.byteLength);
          n += p;
        }
