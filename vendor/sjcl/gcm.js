...
mode.gcm = {
  ...
  encrypt: function encrypt(prf, plaintext, iv, adata, tlen) {
    ...
    // encrypt and tag
    out = mode.gcm._ctrMode(true, prf, data, adata, iv, tlen);
  ...
  decrypt: function decrypt(prf, ciphertext, iv, adata, tlen) {
    ...
    // decrypt and tag
    out = mode.gcm._ctrMode(false, prf, data, adata, iv, tlen);
  ...
  _ghash: function _ghash(H, Y0, data) {
    ...
    for (i = 0; i < l; i += 4) {
      ...
      Yi = mode.gcm._galoisMultiply(Yi, H);
    }
    return Yi;
  },
  /** GCM CTR mode.
  ...
  _ctrMode: function _ctrMode(encrypt, prf, data, adata, iv, tlen) {
    ...
    if (ivbl === 96) {
      ...
      J0 = mode.gcm._ghash(H, [0, 0, 0, 0], iv);
      J0 = mode.gcm._ghash(H, J0, [0, 0, Math.floor(ivbl / 0x100000000), ivbl & 0xffffffff]);
    }
    S0 = mode.gcm._ghash(H, [0, 0, 0, 0], adata);
    ...
    tag = mode.gcm._ghash(H, tag, last);
    enc = prf.encrypt(J0);
    tag[0] ^= enc[0];
    tag[1] ^= enc[1];
    tag[2] ^= enc[2];
