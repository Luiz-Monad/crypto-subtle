import { __module as rasha } from './_virtual/rasha.js';
...
(function (module) {
	...
	RSA.generate = function (opts) {
	  return Promise.resolve().then(function () {
	    ...
	    return new Promise(function (resolve, reject) {
	      return require$$5.generateKeyPair(typ, {
	      ...
	      return {
	        "private": RSA.importSync({
	          pem: keypair["private"],
	          format: priv.type
	        }),
	        "public": RSA.importSync({
	          pem: keypair["public"],
	          format: pub.type,
	          "public": true
	        })
	...
	*/
	...
	RSA.exportSync = function (opts) {
	  ...
	  if (!jwk.p) {
	    ...
	    if (!format || 'pkcs1' === format) {
	      ...
	      throw new Error("options.format must be 'spki', 'pkcs1', or 'ssh' for public RSA keys, not (" + _typeof(format) + ") " + format);
	    }
	  } else {
	...
	RSA.__thumbprint = function (jwk) {
	  var buf = require$$5.createHash('sha256')
	  // alphabetically sorted keys [ 'e', 'kty', 'n' ]
	  .update('{"e":"' + jwk.e + '","kty":"RSA","n":"' + jwk.n + '"}').digest();
